...
function StateBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={positive ? "border-hoikos-200 bg-hoikos-50 text-hoikos-700" : warning ? "border-hoikos-200 bg-hoikos-50 text-hoikos-700" : "border-hoikos-200 bg-hoikos-50 text-hoikos-700"}>
...
function MetricCard({ label, value, note, icon: Icon }: { label: string; value: string; note: string; icon: typeof Clock3 }) {
  return (
    <Card className="workspace-card border-border bg-white/90">
...
function ModuleCard({ title, description, status, icon: Icon, allowed = true }: { title: string; description: string; status: "Ativo" | "Próxima conexão" | "Restrito"; icon: typeof Clock3; allowed?: boolean }) {
  ...
  return (
    <div className="rounded-md border border-hoikos-200/80 bg-white p-4 shadow-none">
      ...
      <p className="mt-1 text-sm leading-6 text-hoikos-500">{allowed ? description : "Seu perfil não pode visualizar esta área."}</p>
    </div>
  );
}
...
export function ProjectHub({ projectId }: { projectId: string }) {
  ...
  return (
    <main className="min-h-svh bg-background">
      ...
      <section className="relative overflow-hidden bg-primary px-4 pb-20 pt-10 text-white sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto -mt-10 max-w-[1500px] px-4 pb-14 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue="resumo" className="mt-8">
          <TabsList variant="line" className="scrollbar-none h-auto w-full justify-start gap-5 overflow-x-auto border-b border-hoikos-200 bg-transparent px-1">
          ...
          <TabsContent value="resumo" className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
            <Card className="workspace-card border-border"><CardHeader><CardTitle className="flex items-center gap-2"><FolderOpen className="size-5 text-hoikos-600" />Ciclo desta obra</CardTitle><p className="text-sm leading-6 text-hoikos-500">Um único ponto para acompanhar planejamento, execução, custos e registros.</p></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><ModuleCard title="Planejamento" description="Datas e tarefas do projeto." status="Ativo" icon={CalendarDays} allowed={can(session, "schedule")} /><ModuleCard title="Tarefas" description={`${state.tasks.length} tarefa(s) vinculada(s).`} status="Ativo" icon={ListChecks} allowed={tasksAllowed} /><ModuleCard title="Orçamento" description="Valor-base e versões do orçamento." status="Ativo" icon={ReceiptText} allowed={budgetAllowed} /><ModuleCard title="Financeiro" description="Integração oficial da empresa." status={state.financial ? "Ativo" : "Próxima conexão"} icon={WalletCards} allowed={financeAllowed} /><ModuleCard title="Diário de obra" description="Fotos, ocorrências, histórico e relatórios na aba Diário e registros." status="Ativo" icon={BookOpenText} allowed={can(session, "diary")} /><ModuleCard title="Arquivos" description="Documentos vinculados ao trabalho." status="Ativo" icon={FileStack} allowed={can(session, "files")} /></div></CardContent></Card>
            <Card className="workspace-card border-border"><CardHeader><CardTitle className="text-base">Ficha do trabalho</CardTitle></CardHeader><CardContent className="space-y-4 text-sm"><InfoRow label="Código" value={project.code} /><InfoRow label="Tipo" value={project.kind === "work" ? "Obra" : "Projeto"} /><InfoRow label="Situação" value={statusLabels[project.status] ?? project.status} /><InfoRow label="Fase" value={project.phase} /><InfoRow label="Responsável" value={project.ownerName ?? "Não definido"} /><InfoRow label="Início" value={formatDate(project.startDate)} /><InfoRow label="Prazo" value={formatDate(project.targetDate)} /></CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="custos" className="mt-6 grid gap-6 lg:grid-cols-2">
            <Card className="workspace-card border-border"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><ReceiptText className="size-5 text-hoikos-600" />Orçamento do projeto</CardTitle></CardHeader><CardContent>{budgetAllowed ? <><p className="metric-number text-4xl font-semibold text-hoikos-950">{project.budgetCents !== null ? currency.format(project.budgetCents / 100) : "Não informado"}</p><p className="mt-3 text-sm leading-6 text-hoikos-500">Valor-base cadastrado. As composições e versões detalhadas já podem ser administradas no módulo Orçamentos.</p><Button asChild variant="outline" className="mt-5"><Link href="/?module=budgets">Abrir orçamentos</Link></Button></> : <HonestMessage icon={LockKeyhole} title="Orçamento restrito" description="Seu nível de acesso não permite visualizar valores desta obra." />}</CardContent></Card>
            <Card className="workspace-card border-border"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><WalletCards className="size-5 text-hoikos-600" />Financeiro da empresa</CardTitle><p className="text-xs leading-5 text-hoikos-500">Estes valores são da empresa inteira, não desta obra isoladamente.</p></CardHeader><CardContent>{!financeAllowed ? <HonestMessage icon={LockKeyhole} title="Financeiro restrito" description="Seu nível de acesso não permite visualizar dados financeiros." /> : state.financial ? <div className="grid grid-cols-2 gap-3"><FinancialValue label="Saldo atual" value={state.financial.currentBalance} /><FinancialValue label="A receber" value={state.financial.receivables} /><FinancialValue label="A pagar" value={state.financial.payables} /><FinancialValue label="Projeção 30 dias" value={state.financial.projected30d} /></div> : <HonestMessage icon={CircleDollarSign} title="Integração financeira pendente" description={state.financialMessage || "Conecte a fonte financeira oficial da empresa para consultar os valores."} />}{financeAllowed ? <><p className="mt-4 text-xs text-hoikos-500">Centro de custo desta obra: {project.externalFinancialCostCenterId ? "vinculado" : "ainda não vinculado"}.</p><Button asChild variant="outline" className="mt-4"><Link href={`/?module=finance&project=${project.id}`}>Abrir financeiro da obra</Link></Button></> : null}</CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="registros" className="mt-6 space-y-6">
            ...
            <Card className="workspace-card border-border"><CardHeader><CardTitle className="flex items-center gap-2"><ClipboardCheck className="size-5 text-hoikos-600" />Outros registros operacionais</CardTitle><p className="text-sm leading-6 text-hoikos-500">As próximas conexões serão ativadas sem inserir dados demonstrativos.</p></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3"><ModuleCard title="Compras" description="Cotações, pedidos e fornecedores." status="Próxima conexão" icon={Boxes} /><ModuleCard title="Medições" description="Evolução física e aprovações." status="Próxima conexão" icon={ClipboardCheck} /><ModuleCard title="Arquivos" description="Projetos, contratos e comprovantes." status="Ativo" icon={FileStack} allowed={can(session, "files")} /></CardContent></Card>
          </TabsContent>
...
function TargetMark({ value }: { value: number }) {
  return <span className="grid size-12 place-items-center rounded-full border border-hoikos-300/20 bg-hoikos-300/10"><span className="size-3 rounded-full bg-hoikos-300 shadow-[0_0_0_6px_rgba(103,232,249,0.12)]" style={{ opacity: Math.max(0.35, value / 100) }} /></span>;
}

function InfoRow({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between gap-5 border-b border-hoikos-100 pb-3 last:border-0 last:pb-0"><span className="text-hoikos-500">{label}</span><span className="text-right font-medium text-hoikos-900">{value}</span></div>;
}

function TimelinePoint({ title, value, active = false }: { title: string; value: string; active?: boolean }) {
  return <div className="relative"><span className={`absolute -left-8 top-1 grid size-6 place-items-center rounded-full border-4 border-white ${active ? "bg-hoikos-600" : "bg-hoikos-300"}`} /><p className="text-xs font-semibold uppercase tracking-[0.14em] text-hoikos-500">{title}</p><p className="mt-1 font-medium text-hoikos-900">{value}</p></div>;
}
