...
/** Campo `type="number"`: o navegador já entrega com ponto decimal ("0.125"), mesmo em
 *  pt-BR. Passar isso pelo leitor brasileiro lia o ponto como milhar — 0,125 m³ virava 125. */
function inputNumber(value: FormDataEntryValue | null, fallback = 0) {
  const parsed = Number(String(value ?? "").trim());
  return String(value ?? "").trim() !== "" && Number.isFinite(parsed) ? parsed : fallback;
}

/** Texto digitado ou colado (importação em lote): aí vale a convenção brasileira. */
...
export function BudgetsWorkspace({ projects, query, canEdit }: { projects: Project[]; query: string; canEdit: boolean }) {
  ...
  async function createBudget(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await requestJson<{ budget: Budget }>("/api/budgets", { method: "POST", body: JSON.stringify({ projectId: data.get("projectId"), code: data.get("code"), bdiPercent: inputNumber(data.get("bdiPercent")), marginPercent: inputNumber(data.get("marginPercent")) }) });
  ...
  async function addItems(nextItems: DraftItem[]) {
    if (saving) throw new Error("Aguarde o envio anterior terminar.");
    if (!canEdit || !selected || selected.status !== "draft" || nextItems.length === 0) {
      // Antes o erro só ia para o console: o clique parecia não fazer nada.
      const motivo = "Selecione um rascunho disponível para adicionar itens.";
      toast.error(motivo);
  ...
  async function addManual(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(event.currentTarget);
    const item: DraftItem = { code: String(data.get("code") || "") || null, description: String(data.get("description") || ""), unit: String(data.get("unit") || "un"), quantity: inputNumber(data.get("quantity"), 1), unitCostCents: moneyToCents(data.get("unitCost")), unitPriceCents: data.get("unitPrice") ? moneyToCents(data.get("unitPrice")) : null, source: "manual" };
    try {
      await addItems([item]);
  ...
  return (
    <div className="space-y-5">
      ...
      <Tabs defaultValue="budgets">
        <TabsList className="rounded-md"><TabsTrigger value="budgets"><Calculator />Orçamentos</TabsTrigger><TabsTrigger value="library"><LibraryBig />Biblioteca <Badge variant="secondary">{library.length}</Badge></TabsTrigger><TabsTrigger value="sinapi"><FileSpreadsheet />Relatórios SINAPI</TabsTrigger></TabsList>
        <TabsContent value="sinapi" className="mt-5"><SinapiReferenceBrowser /></TabsContent>
