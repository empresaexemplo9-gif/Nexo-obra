...
function PortalFrame({ children }: { children: React.ReactNode }) {
  return <main className="min-h-svh bg-[#edf2f8]"><header className="border-b border-white/10 bg-[#08172c] text-white"><div className="mx-auto flex max-w-6xl flex-wrap items-center gap-4 px-4 py-5 sm:px-6"><Image src="/drap-architector-logo.png" alt="Drap Architector" width={2037} height={772} priority className="h-auto w-44" /><span className="ml-auto text-sm text-cyan-200">Portal do cliente</span><Button asChild variant="ghost" size="icon" aria-label="Sair do portal"><a href="/signout-with-chatgpt?return_to=%2Fportal" target="_top"><LogOut className="size-4" /></a></Button></div></header><div className="mx-auto max-w-6xl space-y-6 px-4 py-7 sm:px-6">{children}</div></main>;
}
function SignInCard({ returnTo }: { returnTo: string }) {
  return <Card className="mx-auto mt-10 max-w-lg"><CardContent className="space-y-5 p-7"><ClipboardCheck className="size-10 text-blue-600" /><h1 className="text-2xl font-semibold">Acompanhe sua obra</h1><p className="text-base leading-7 text-slate-600">Entre com a conta ChatGPT do mesmo e-mail que recebeu o convite da empresa. O convite não libera acesso para outra pessoa.</p><Button asChild className="w-full"><a href={`/signin-with-chatgpt?return_to=${encodeURIComponent(returnTo)}`} target="_top">Entrar com ChatGPT</a></Button><p className="text-sm text-slate-500">Ainda não recebeu um convite? Solicite à empresa responsável pela obra.</p></CardContent></Card>;
}
export function ClientPortalApp() {
  ...
  return <PortalFrame>{session.error && <PortalError message={session.error} retry={session.refresh} />}{session.loading ? <Skeleton className="h-64 rounded-2xl" /> : session.data && !session.data.authenticated ? <SignInCard returnTo="/portal" /> : session.data?.authenticated ? <>
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-slate-500">Bem-vindo, {session.data.userName}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Suas obras e decisões</h1></div><Button variant="outline" onClick={session.refresh}><RefreshCw className="size-4" />Atualizar</Button></div>
...
function PortalTerms({ endpoint, version, onAccepted, buttonText = "Aceitar e continuar" }: { endpoint: string; version: string; onAccepted: () => void; buttonText?: string }) {
  ...
  return <Card><CardContent className="space-y-4 p-6"><h2 className="text-xl font-semibold">Confirme seu acesso</h2><p className="text-base leading-7 text-slate-600">Você verá somente as obras e os conteúdos que a empresa compartilhar. Aprovações e pedidos de ajuste ficam registrados com sua identidade, data e conteúdo apresentado.</p><div className="flex items-start gap-3"><Checkbox id="portal-terms" checked={checked} onCheckedChange={(value) => setChecked(value === true)} disabled={busy} /><Label htmlFor="portal-terms" className="block leading-6">Li e aceito os <a href="/termos" target="_blank" rel="noreferrer" className="text-blue-700 underline">Termos de Uso</a> (versão {version}).</Label></div>{error && <PortalError message={error} />}<Button disabled={!checked || busy} onClick={() => void accept()}>{busy && <LoaderCircle className="size-4 animate-spin" />}{buttonText}</Button></CardContent></Card>;
}
...
function ClientProject({ access, onAccessChanged }: { access: PortalAccess; onAccessChanged: () => void }) {
  ...
  return <div className="space-y-5">{resource.error && <PortalError message={resource.error} retry={resource.refresh} />}{data.progress && <Card className="border-0 bg-[#0b213d] text-white"><CardContent className="grid gap-6 p-6 md:grid-cols-[1fr_1fr]"><div><p className="text-sm text-cyan-200">{data.access.projectCode} · {statuses[data.progress.status] ?? data.progress.status}</p><h2 className="mt-2 text-2xl font-semibold">{data.access.projectName}</h2><p className="mt-2 text-base text-slate-300">Fase: {data.progress.phase}</p><p className="mt-2 text-sm text-slate-300">Início: {data.progress.startDate ? diaryDate(data.progress.startDate) : "Não informado"} · Entrega prevista: {data.progress.targetDate ? diaryDate(data.progress.targetDate) : "Não informada"}</p></div><div className="self-center"><div className="flex justify-between text-sm"><span>Avanço informado pela empresa</span><strong className="text-xl text-cyan-300">{data.progress.progressPercent}%</strong></div><Progress value={data.progress.progressPercent} className="mt-3 bg-white/10 [&_[data-slot=progress-indicator]]:bg-cyan-300" /></div></CardContent></Card>}
    <div className="flex flex-wrap justify-between gap-2"><h2 className="text-xl font-semibold">{data.pending ? `${data.pending} aprovação(ões) pendente(s)` : "Atualizações e histórico"}</h2><p className="text-sm text-slate-500">Atualização automática · 15 s</p></div>
    {data.items.map((item) => <PortalItemCard key={item.id} item={item}>{item.kind === "approval" && item.status === "open" && (data.access.canApprove && !resource.error ? <DecisionForm item={item} onSaved={resource.refresh} /> : <p className="text-sm text-slate-500">A resposta depende da permissão de aprovação liberada pela empresa.</p>)}</PortalItemCard>)}
...
function DecisionForm({ item, onSaved }: { item: PortalItem; onSaved: () => void }) {
  ...
  return <form onSubmit={submit} className="space-y-4 border-t border-slate-200 pt-4"><fieldset disabled={busy} className="space-y-4"><div className="space-y-2"><Label htmlFor={`choice-${item.id}`}>Sua decisão</Label><Select value={choice} onValueChange={(value) => { setChoice(value as typeof choice); setConfirmed(false); }}><SelectTrigger id={`choice-${item.id}`} className="w-full"><SelectValue placeholder="Escolha uma resposta" /></SelectTrigger><SelectContent><SelectItem value="approved">Aprovar o conteúdo apresentado</SelectItem><SelectItem value="changes_requested">Solicitar ajustes</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label htmlFor={`comment-${item.id}`}>{choice === "changes_requested" ? "Quais ajustes são necessários?" : "Observação (opcional)"}</Label><Textarea id={`comment-${item.id}`} rows={3} maxLength={3000} required={choice === "changes_requested"} minLength={choice === "changes_requested" ? 5 : undefined} value={comment} onChange={(event) => setComment(event.target.value)} /></div><div className="flex items-start gap-3"><Checkbox id={`confirm-${item.id}`} checked={confirmed} onCheckedChange={(value) => setConfirmed(value === true)} /><Label htmlFor={`confirm-${item.id}`} className="block leading-6">Revisei o conteúdo e confirmo o registro desta decisão. Ela ficará preservada no histórico.</Label></div><p className="text-xs text-slate-500">Esta confirmação registra sua decisão na plataforma; não é uma assinatura digital certificada.</p></fieldset>{error && <PortalError message={error} />}<Button disabled={!choice || !confirmed || busy} type="submit">{busy ? <LoaderCircle className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}Registrar decisão</Button></form>;
}
