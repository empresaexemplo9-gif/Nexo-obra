...
function Brand({ compact = false, dark = false, stacked = false }: { compact?: boolean; dark?: boolean; stacked?: boolean }) {
  return <BrandLogo compact={compact} dark={dark || compact} stacked={stacked} />;
}

function LoadingScreen() {
  return <main className="grid min-h-svh place-items-center bg-hoikos-950"><div className="flex flex-col items-center gap-5"><Brand dark stacked /><LoaderCircle className="size-6 animate-spin text-hoikos-500" /><p className="text-sm text-hoikos-500">Abrindo sua empresa…</p></div></main>;
}
    ...
    <main className="relative grid min-h-svh place-items-center overflow-hidden bg-primary p-5">
      ...
      <Card className="relative w-full max-w-lg border-white/10 bg-primary text-white shadow-none ">
        <CardContent className="p-7 sm:p-9">
          ...
          <Button asChild className="mt-3 h-11 w-full rounded-xl bg-hoikos-500 text-hoikos-950 hover:bg-hoikos-400">
          ...
          <div className="my-7 flex items-center gap-3 text-xs uppercase tracking-[0.18em] text-hoikos-600"><span className="h-px flex-1 bg-white/10" />Superadmin<span className="h-px flex-1 bg-white/10" /></div>
          <form onSubmit={submitSuperadmin} className="space-y-4">
            <div><label htmlFor="initial-superadmin-email" className="mb-2 block text-sm font-medium text-hoikos-200">E-mail</label><Input id="initial-superadmin-email" type="email" value={adminEmail} onChange={(event) => setAdminEmail(event.target.value)} autoComplete="username" required className="h-11 border-white/10 bg-white/[0.06] text-white placeholder:text-hoikos-600" /></div>
            <div><label htmlFor="initial-superadmin-password" className="mb-2 block text-sm font-medium text-hoikos-200">Senha</label><div className="relative"><Input id="initial-superadmin-password" type={showAdminPassword ? "text" : "password"} value={adminPassword} onChange={(event) => setAdminPassword(event.target.value)} autoComplete="current-password" required className="h-11 border-white/10 bg-white/[0.06] pr-11 text-white placeholder:text-hoikos-600" /><button type="button" onClick={() => setShowAdminPassword((value) => !value)} aria-label={showAdminPassword ? "Ocultar senha" : "Mostrar senha"} className="absolute right-3 top-1/2 -translate-y-1/2 text-hoikos-500 hover:text-white">{showAdminPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div></div>
            {adminError ? <p role="alert" className="rounded-xl border border-hoikos-400/20 bg-hoikos-400/10 px-3 py-2 text-sm text-hoikos-200">{adminError}</p> : null}
            <Button type="submit" variant="outline" disabled={adminLoading} className="h-11 w-full rounded-xl border-hoikos-300/25 bg-hoikos-300/10 text-hoikos-100 hover:bg-hoikos-200/20 hover:text-white">{adminLoading ? <LoaderCircle className="animate-spin" /> : <KeyRound />}Entrar como superadmin</Button>
          </form>
          <p className="mt-6 flex items-center justify-center gap-2 text-xs text-hoikos-500"><ShieldCheck className="size-4" />Acesso protegido e dados separados por empresa.</p><div className="mt-3 flex items-center justify-center gap-4 text-xs"><a href="/termos" className="text-hoikos-300 hover:text-hoikos-200">Termos de Uso</a><a href="/manutencao" className="text-hoikos-300 hover:text-hoikos-200">Acesso de manutenção</a></div>
        </CardContent>
      </Card>
    </main>
  );
...
function OrganizationForm({ onCreated, embedded = false }: { onCreated: () => Promise<void> | void; embedded?: boolean }) {
  ...
  const form = (
    <form onSubmit={submit} className="space-y-5">
      ...
      <label className="flex items-start gap-3 rounded-xl border border-hoikos-200 bg-hoikos-50 p-4 text-sm leading-6"><Checkbox checked={acceptedTerms} onCheckedChange={(value) => setAcceptedTerms(value === true)} className="mt-1" /><span>Li e aceito os <a href="/termos" target="_blank" className="font-medium text-hoikos-700 underline underline-offset-2">Termos de Uso e Privacidade</a>.</span></label>
      {error ? <p role="alert" className="text-sm text-hoikos-600">{error}</p> : null}
      <Button type="submit" disabled={saving || name.trim().length < 2 || !acceptedTerms} className="h-11 w-full rounded-xl bg-hoikos-500 text-hoikos-950 hover:bg-hoikos-400">{saving ? <LoaderCircle className="animate-spin" /> : <Building2 />}Criar empresa</Button>
    </form>
  );
  if (embedded) return form;
  return (
    <main className="grid min-h-svh place-items-center bg-hoikos-50 p-5">
      <Card className="w-full max-w-lg border-hoikos-200 shadow-none"><CardHeader><Brand /><CardTitle className="pt-6 text-2xl">Crie sua primeira empresa</CardTitle><p className="text-sm leading-6 text-hoikos-500">A conta começa vazia. Você adiciona apenas os dados reais da sua operação.</p></CardHeader><CardContent>{form}</CardContent></Card>
    </main>
  );
}
...
function Kpi({ icon: Icon, label, value, detail, tone = "blue" }: { icon: LucideIcon; label: string; value: string; detail: string; tone?: "blue" | "amber" | "emerald" }) {
  const tones = { blue: "bg-hoikos-50 text-hoikos-700", amber: "bg-hoikos-50 text-hoikos-700", emerald: "bg-hoikos-50 text-hoikos-700" };
  return <Card className="p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-hoikos-500">{label}</p><p className="metric-number mt-2 text-2xl font-semibold text-hoikos-950">{value}</p><p className="mt-1 text-xs text-hoikos-500">{detail}</p></div><span className={`grid size-10 place-items-center rounded-xl ${tones[tone]}`}><Icon className="size-5" /></span></div></Card>;
}
...
function PageIntro({ module, action, actionLabel }: { module: ModuleId; action?: () => void; actionLabel?: string }) {
  ...
  return <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-hoikos-600">Empresa atual</p><h1 className="display-heading mt-2 text-3xl font-semibold text-hoikos-950">{copy.title}</h1><p className="mt-2 text-sm text-hoikos-500">{copy.description}</p></div>{action ? <Button onClick={action} className="rounded-xl"><Plus />{actionLabel}</Button> : null}</div>;
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  const color = status === "completed" || status === "done" ? "border-hoikos-200 bg-hoikos-50 text-hoikos-700" : status === "blocked" || status === "on_hold" ? "border-hoikos-200 bg-hoikos-50 text-hoikos-700" : "border-hoikos-200 bg-hoikos-50 text-hoikos-700";
...
function Field({ name, label, type = "text", placeholder, required }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return <div><label htmlFor={name} className="mb-1.5 block text-sm font-medium text-hoikos-700">{label}</label><Input id={name} name={name} type={type} placeholder={placeholder} required={required} /></div>;
}
function NativeChoice({ name, label, options, defaultValue, required }: { name: string; label: string; options: { value: string; label: string }[]; defaultValue?: string; required?: boolean }) {
  return <div><label htmlFor={name} className="mb-1.5 block text-sm font-medium text-hoikos-700">{label}</label><select id={name} name={name} defaultValue={defaultValue} required={required} className="h-9 w-full rounded-md border border-hoikos-200 bg-white px-3 text-sm outline-none focus:border-hoikos-500 focus:ring-2 focus:ring-hoikos-100">{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></div>;
}
...
function Workspace({ session, reloadSession }: { session: SessionData; reloadSession: () => Promise<void> }) {
  ...
  return <SidebarProvider className="nexo-shell" style={{ "--sidebar-width": "17rem" } as React.CSSProperties}>
    ...
    <SidebarInset><header className="nexo-header sticky top-0 z-20 flex h-[4.5rem] items-center gap-3 border-b border-border px-4  sm:px-6"><SidebarTrigger className="size-10 rounded-xl border border-hoikos-200 bg-white" /><p className="hidden text-sm font-semibold text-hoikos-700 sm:block">{moduleTitles[activeModule].title}</p>{session.authMethod === "maintenance" ? <Badge className="hidden border-hoikos-200 bg-hoikos-50 text-hoikos-800 sm:inline-flex">Ambiente de manutenção</Badge> : null}<div className="mx-auto w-full max-w-md sm:ml-auto sm:mr-0"><div className="relative"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-hoikos-500" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar nesta empresa…" className="h-10 rounded-xl bg-white pl-10" /></div></div><Button variant="ghost" size="icon" aria-label="Notificações"><Bell /></Button>{canCreateAny ? <Button size="sm" onClick={() => openCreate(canEdit("projects") ? "project" : canEdit("crm") ? "client" : "task")} className="rounded-xl"><Plus /><span className="hidden sm:inline">Criar</span></Button> : null}</header><main className="nexo-canvas min-h-[calc(100svh-4.5rem)] p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1480px]">{content}</div></main></SidebarInset>
