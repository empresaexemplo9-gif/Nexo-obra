...
export function PortalError({ message, retry }: { message: string; retry?: () => void }) {
  return <div role="alert" className="flex flex-wrap items-center gap-3 rounded-xl border border-hoikos-200 bg-hoikos-50 p-4 text-sm text-hoikos-950"><CircleAlert className="size-5 shrink-0" /><p className="min-w-0 flex-1">{message}</p>{retry && <Button variant="outline" size="sm" onClick={retry}>Tentar novamente</Button>}</div>;
}
