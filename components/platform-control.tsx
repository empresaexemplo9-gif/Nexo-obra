  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Ativação e mensalidade</CardTitle></CardHeader><CardContent className="space-y-4">
        ...
        {!data.configured && <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-900">A conexão de ativação com o Empresa ainda não está configurada. Nenhuma cobrança foi iniciada por esta ferramenta.</p>}
        {data.activation ? <>
          ...
          {data.activation.last_error && <p className="text-sm text-amber-900">{data.activation.last_error}</p>}
        ...
        </> : <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); setConfirmation({ body: { action: "enroll", companyId, planId, confirmed: true }, text: `Vincular ${name} ao plano ${planId} do Empresa. O acesso ficará aguardando confirmação da assinatura. A mensalidade será o valor do plano acrescido de 50%.` }); }}>
    ...
    <Card><CardHeader><CardTitle>Controle de acessos</CardTitle></CardHeader><CardContent className="space-y-5">
      <p className="text-sm text-slate-600">Bloquear ou excluir um acesso não cancela a assinatura. Gerencie o cancelamento da cobrança no Drap Empresa.</p>
      ...
      <div className="grid gap-3 md:grid-cols-2">{data.members.length === 0 && <p>Nenhum membro cadastrado nesta empresa.</p>}{data.members.map((member) => {
        ...
        return <div className="rounded-lg border p-4" key={member.id}><p className="font-medium">{member.name}</p><p className="break-all text-sm text-slate-600">{member.email} · {labels[member.role] ?? member.role}</p><p className="mt-2 text-sm">{rule ? labels[rule.state] : member.active ? "Ativo" : "Inativo"}{rule?.until ? ` até ${new Date(rule.until).toLocaleString("pt-BR")}` : ""}</p><Button className="mt-2" size="sm" variant="outline" onClick={() => setSubject(member.email.toLowerCase())}>Selecionar acesso</Button></div>;
      })}</div>
