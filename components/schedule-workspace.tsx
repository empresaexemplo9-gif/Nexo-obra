...
export function ScheduleWorkspace({ projects, tasks, query, canEdit, onChanged }: {
  ...
  const now = Date.now();
  const late = tasks.filter((task) => task.status !== "done" && task.dueAt && new Date(task.dueAt).getTime() < now).length;
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await patchTask(editing.id, {
        startsAt, dueAt,
        estimatedMinutes: Math.max(0, Math.round(Number(data.get("estimatedMinutes") || 0))),
        parentTaskId: data.get("parentTaskId") === "none" ? null : data.get("parentTaskId"),
      });
  ...
  return <div className="space-y-5">
    <div><p className="eyebrow text-hoikos-600">Planejamento</p><h1 className="display-heading mt-2 text-4xl text-hoikos-950">Cronograma</h1><p className="mt-2 text-sm text-hoikos-500">Início, prazo, duração e dependência usam as próprias tarefas da operação.</p></div>
