      ...
      <Card className="relative w-full max-w-md overflow-hidden border-white/10 bg-primary text-white shadow-none ">
        ...
        <CardContent className="p-7 sm:p-9">
          ...
          <div className="mt-8 flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-xl bg-hoikos-400/10 text-hoikos-300 ring-1 ring-hoikos-300/20"><ShieldCheck className="size-5" /></span>
            <div><h1 className="text-2xl font-semibold tracking-tight">Controle da plataforma</h1><p className="mt-1 text-sm text-hoikos-500">Acesso exclusivo do superadministrador.</p></div>
          </div>
          <form onSubmit={submit} className="mt-8 space-y-4">
            ...
            {error ? <p role="alert" className="rounded-xl border border-hoikos-400/20 bg-hoikos-400/10 px-3 py-2 text-sm text-hoikos-200">{error}</p> : null}
            <Button type="submit" disabled={submitting} className="h-11 w-full rounded-xl bg-hoikos-400 text-hoikos-950 hover:bg-hoikos-200">{submitting ? <LoaderCircle className="animate-spin" /> : <KeyRound />}Entrar como superadmin</Button>
          </form>
  ...
  return (
    <main className="min-h-svh bg-hoikos-100">
      ...
      <div className=" mx-auto max-w-[1480px] px-5 py-7 sm:px-8 sm:py-10">
        <div className="mb-7"><div className="flex items-center gap-2 text-sm font-medium text-hoikos-700"><ShieldCheck className="size-4" />Visão global protegida</div><h1 className="display-heading mt-2 text-3xl font-semibold tracking-tight text-hoikos-950 sm:text-4xl">Controle da plataforma</h1><p className="mt-2 text-hoikos-600">Acompanhe metadados e indicadores de uso. O conteúdo confidencial de cada empresa não é exibido ao superadmin.</p></div>
