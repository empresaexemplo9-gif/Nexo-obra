...
type Invitation = { email: string; role: string; organizationName: string; expiresAt: number };
...
export function InvitationApp({ token }: { token: string }) {
  ...
  async function accept() {
    ...
    setError("");
    try {
      await readJson(`/api/invitations/${encodeURIComponent(token)}/accept`, { method: "POST" });
      setAccepted(true);
    } catch (cause) {
