...
export function TermsPage() {
  return <main className="min-h-svh bg-hoikos-100"><header className="border-b border-hoikos-800 bg-primary text-white"><div className="mx-auto flex max-w-5xl items-center gap-4 px-5 py-5 sm:px-8"><BrandLogo variant="lockup" dark className="w-[210px]" /><Link href="/" className="ml-auto text-sm text-hoikos-300 hover:text-hoikos-300">Voltar à plataforma</Link></div></header><article className="mx-auto max-w-4xl px-5 py-10 sm:px-8 sm:py-14"><p className="eyebrow text-hoikos-700">Versão {CURRENT_TERMS_VERSION}</p><h1 className="display-heading mt-3 text-5xl text-hoikos-950">Termos de Uso da H.OIKOS</h1><p className="mt-3 text-hoikos-600">Vigentes a partir de {CURRENT_TERMS_EFFECTIVE_DATE}.</p><div className="mt-8 rounded-md border border-hoikos-200 bg-hoikos-50 p-5 text-sm leading-6 text-hoikos-950"><strong>Documento inicial.</strong> Estes Termos organizam o funcionamento do produto e o aceite eletrônico, mas devem ser revisados por assessoria jurídica antes do início da cobrança comercial e complementados pelos dados do fornecedor, política de privacidade, canal de atendimento e condições do plano contratado.</div><div className="mt-10 space-y-9">{sections.map((section) => <section key={section.title}><h2 className="text-xl font-semibold text-hoikos-950">{section.title}</h2><div className="mt-3 space-y-3 text-base leading-7 text-hoikos-700">{section.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>)}</div><section className="mt-10 border-t border-hoikos-200 pt-8"><h2 className="text-xl font-semibold text-hoikos-950">Referências normativas</h2><p className="mt-3 leading-7 text-hoikos-700">Este texto considera a Lei Geral de Proteção de Dados Pessoais, o Marco Civil da Internet, o Código de Defesa do Consumidor quando aplicável e orientações da Autoridade Nacional de Proteção de Dados. A referência não substitui avaliação jurídica do modelo comercial concreto.</p><ul className="mt-4 space-y-2 text-sm text-hoikos-700"><li><a className="hover:underline" href="https://www.planalto.gov.br/ccivil_03/_ato2015-2018/2018/lei/l13709compilado.htm" target="_blank" rel="noreferrer">Lei Geral de Proteção de Dados Pessoais</a></li><li><a className="hover:underline" href="https://www.planalto.gov.br/ccivil_03/_ato2011-2014/2014/lei/l12965.htm" target="_blank" rel="noreferrer">Marco Civil da Internet</a></li><li><a className="hover:underline" href="https://www.planalto.gov.br/ccivil_03/leis/l8078compilado.htm" target="_blank" rel="noreferrer">Código de Defesa do Consumidor</a></li><li><a className="hover:underline" href="https://www.gov.br/anpd/pt-br/centrais-de-conteudo/materiais-educativos-e-publicacoes/guia-orientativo-sobre-seguranca-da-informacao-para-agentes-de-tratamento-de-pequeno-porte" target="_blank" rel="noreferrer">Guia de Segurança da Informação da ANPD</a></li></ul></section></article></main>;
}
