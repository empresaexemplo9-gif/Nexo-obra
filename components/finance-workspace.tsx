...
export function FinanceWorkspace({ projects, query, canEdit, canManageConnection, onProjectsChanged }: { projects: Project[]; query: string; canEdit: boolean; canManageConnection: boolean; onProjectsChanged: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-hoikos-600">Empresa atual</p><h1 className="display-heading mt-2 text-3xl font-semibold text-hoikos-950">Financeiro</h1><p className="mt-2 text-sm text-hoikos-500">Contas, cobranças e resultado no contexto de cada obra.</p></div><div className="flex flex-wrap gap-2">{canManageConnection ? <Button variant="outline" onClick={() => setSettingsOpen(true)}><Settings2 />Conexão Drap</Button> : null}{canEdit ? <Button onClick={() => setChargeOpen(true)} disabled={!connection || !capabilities.charges}><Plus />Nova cobrança</Button> : null}</div></div>
...
function Metric({ label, value, icon: Icon, tone }: { label: string; value: string; icon: typeof Building2; tone: "blue" | "emerald" | "amber" }) {
  ...
  return <Card className="workspace-card border-border"><CardContent className="flex items-start justify-between gap-4 p-5"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-hoikos-500">{label}</p><p className="metric-number mt-3 text-2xl font-semibold text-hoikos-950">{value}</p></div><span className={`grid size-10 place-items-center rounded-xl ${color}`}><Icon className="size-5" /></span></CardContent></Card>;
}

function Capability({ label, enabled }: { label: string; enabled: boolean }) {
  return <div className={`rounded-xl border p-3 ${enabled ? "border-hoikos-200 bg-hoikos-50 text-hoikos-800" : "border-hoikos-200 bg-hoikos-50 text-hoikos-500"}`}><p className="font-medium">{label}</p><p className="mt-1">{enabled ? "Disponível" : "Pendente"}</p></div>;
}
