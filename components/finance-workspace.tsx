...
export function FinanceWorkspace({ projects, query, canEdit, canManageConnection, onProjectsChanged }: { projects: Project[]; query: string; canEdit: boolean; canManageConnection: boolean; onProjectsChanged: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow text-hoikos-600">Empresa atual</p><h1 className="display-heading mt-2 text-4xl text-hoikos-950">Financeiro</h1><p className="mt-2 text-sm text-hoikos-500">Contas, cobranças e resultado no contexto de cada obra.</p></div><div className="flex flex-wrap gap-2">{canManageConnection ? <Button variant="outline" onClick={() => setSettingsOpen(true)}><Settings2 />Conexão Drap</Button> : null}{canEdit ? <Button onClick={() => setChargeOpen(true)} disabled={!connection || !capabilities.charges}><Plus />Nova cobrança</Button> : null}</div></div>
