...
const DRAP_PORTAL_URL = process.env.NEXT_PUBLIC_DRAP_PORTAL_URL || "https://empresa.drap.app.br/inicio";
...
export function FinanceWorkspace({ projects, query, canEdit, canManageConnection, onProjectsChanged }: { projects: Project[]; query: string; canEdit: boolean; canManageConnection: boolean; onProjectsChanged: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow text-hoikos-600">Empresa atual</p><h1 className="display-heading mt-2 text-4xl text-hoikos-950">Financeiro</h1><p className="mt-2 text-sm text-hoikos-500">Contas, cobranças e resultado no contexto de cada obra.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline"><a href={DRAP_PORTAL_URL} target="_blank" rel="noopener noreferrer"><ExternalLink />Abrir Drap</a></Button>{canManageConnection ? <Button variant="outline" onClick={() => setSettingsOpen(true)}><Settings2 />Conexão Drap</Button> : null}{canEdit ? <Button onClick={() => setChargeOpen(true)} disabled={!connection || !capabilities.charges}><Plus />Nova cobrança</Button> : null}</div></div>
      ...
      <Tabs defaultValue="accounts">
        <TabsList className="rounded-md"><TabsTrigger value="accounts"><CircleDollarSign />Contas</TabsTrigger><TabsTrigger value="charges"><Send />Cobranças <Badge variant="secondary">{charges.length}</Badge></TabsTrigger><TabsTrigger value="reports"><Download />Relatórios</TabsTrigger></TabsList>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent><DialogHeader><DialogTitle>Conexão da empresa com a Drap</DialogTitle><DialogDescription>Informe apenas o identificador da empresa. Token e segredo permanecem no servidor da plataforma. O botão “Abrir Drap” usa o portal operacional em {DRAP_PORTAL_URL}.</DialogDescription></DialogHeader><form onSubmit={saveConnection} className="space-y-4"><Field name="externalCompanyId" label="ID da empresa na Drap" defaultValue={connection?.externalCompanyId ?? ""} required /><div className="grid grid-cols-3 gap-2 text-center text-xs"><Capability label="Resumo" enabled={capabilities.summary} /><Capability label="Contas" enabled={capabilities.transactions} /><Capability label="Cobranças" enabled={capabilities.charges} /></div><Button type="submit" disabled={saving} className="w-full">{saving ? <LoaderCircle className="animate-spin" /> : <Check />}Salvar conexão</Button></form></DialogContent></Dialog>
...
function Metric({ label, value, icon: Icon, tone }: { label: string; value: string; icon: typeof Building2; tone: "blue" | "emerald" | "amber" }) {
  const color = tone === "emerald" ? "bg-hoikos-50 text-hoikos-700" : tone === "amber" ? "bg-hoikos-50 text-hoikos-700" : "bg-hoikos-50 text-hoikos-700";
  return <Card className="workspace-card border-border"><CardContent className="flex items-start justify-between gap-4 p-5"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-hoikos-500">{label}</p><p className="metric-number mt-3 text-2xl font-semibold text-hoikos-950">{value}</p></div><span className={`grid size-10 place-items-center rounded-md ${color}`}><Icon className="size-5" /></span></CardContent></Card>;
}
