...
test("SINAPI vazia mostra configuração com controles rotulados e sem preços fictícios", async () => {
  ...
  // Regex e não texto exato: o rótulo já foi "Preparar referência" e virou
  // "Baixar e preparar referência oficial" quando a importação nacional
  // entrou (ef64afe). O que o teste protege é o controle existir e estar
  // nomeado pelo que faz — não a frase inteira, que muda com o produto.
  assert.ok(findByText(container, /preparar referência/i));
});
