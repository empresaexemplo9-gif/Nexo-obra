...
const runtime = {};
globalThis.__diaryTestRuntime = runtime;
const vite = await createServer({ appType: "custom", configFile: false, root, resolve: { alias: { "@": root } },
  plugins: [{ name: "test-cloudflare-bindings", resolveId(id) { if (id === "cloudflare:workers") return "\0diary-test-runtime"; }, load(id) { if (id === "\0diary-test-runtime") return "export const env = globalThis.__diaryTestRuntime;"; } }], server: { middlewareMode: true } });
...
after(async () => { db?.sqlite.close(); await vite.close(); delete globalThis.__diaryTestRuntime; });
