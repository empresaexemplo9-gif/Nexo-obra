...
const FILTERS = ["tipo", "ativo", "limit", "offset"] as const;
...
export async function GET(request: Request) {
  return drapOperationalRoute(async () => {
    ...
    return callOperationalDrap(connection.external_company_id, `/api/v1/parceiros${suffix}`);
  });
}
...
export async function POST(request: Request) {
  return drapOperationalRoute(async () => {
    ...
    return callOperationalDrap(connection.external_company_id, "/api/v1/parceiros", { method: "POST", body, idempotencyKey });
  });
}
