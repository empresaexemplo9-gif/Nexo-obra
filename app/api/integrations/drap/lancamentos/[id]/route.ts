...
export async function PATCH(request: Request, { params }: RouteContext) {
  return drapOperationalRoute(async () => {
    ...
    return callOperationalDrap(connection.external_company_id, `/api/v1/lancamentos/${encodeURIComponent(id)}`, { method: "PATCH", body });
  });
}
...
export async function DELETE(request: Request, { params }: RouteContext) {
  return drapOperationalRoute(async () => {
    const { connection } = await operationalDrapContext(request, "edit");
    const { id } = await params;
    return callOperationalDrap(connection.external_company_id, `/api/v1/lancamentos/${encodeURIComponent(id)}`, { method: "DELETE" });
  });
}
