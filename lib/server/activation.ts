...
export function activationConfigured() {
  const c = config();
  return Boolean(c.DRAP_ACTIVATION_URL && c.DRAP_ACTIVATION_TOKEN && c.DRAP_ACTIVATION_WEBHOOK_SECRET && c.DRAP_ACTIVATION_WEBHOOK_SECRET.length >= 32);
}

function checkedMonthlyCents(base: number) {
...
// Mantido apenas para validar confirmações de ativações antigas já emitidas pelo contrato
// anterior. Novas solicitações nunca usam este cálculo.
export function architectorMonthlyCents(base: number) {
  checkedMonthlyCents(base);
...
// A DRAP continua dona do catálogo, cobrança, faturas e estado financeiro da assinatura.
// Este adaptador só é habilitado quando existir um endpoint de serviço homologado no
// domínio empresa.drap.app.br. A API pública /api/v1 não documenta provisionamento.
export async function sendActivation(row: Activation) {
  const c = config();
  if (!activationConfigured()) throw new ApiError(503, "activation_not_configured", "A API de contratação embutida da DRAP ainda não foi homologada.");
  const url = new URL(c.DRAP_ACTIVATION_URL!);
  if (url.protocol !== "https:" || url.username || url.password || url.hostname !== "empresa.drap.app.br") {
    throw new ApiError(503, "activation_invalid_url", "Configure um endpoint HTTPS homologado da DRAP.");
  }
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        Authorization: `Bearer ${c.DRAP_ACTIVATION_TOKEN}`,
        "Content-Type": "application/json",
        "Idempotency-Key": row.request_key,
      },
      body: JSON.stringify({
    ...
    await db.prepare("UPDATE drap_activations SET last_error = ?1 WHERE organization_id = ?2")
      .bind("Confirmação pendente. Reenviar reutiliza a mesma solicitação.", row.organization_id).run();
...
const eventSchema = z.object({
  ...
  baseMonthlyCents: z.number().int().min(0).max(100_000_000),
  monthlyCents: z.number().int().min(0).max(150_000_000),
  currency: z.literal("BRL"),
  interval: z.literal("month"),
  product: z.enum(["drap_embedded", "drap_architector"]),
  billingOwner: z.literal("drap_empresa"),
}).strict();
...
export async function receiveActivation(request: Request) {
  ...
  const signatureHeader = request.headers.get("x-drap-signature") ?? "";
  ...
  if (!/^\d{10}$/.test(timestamp) || Math.abs(Date.now() / 1000 - Number(timestamp)) > 300 || !/^[a-f0-9]{64}$/.test(signature)) {
    throw new ApiError(401, "invalid_signature", "Assinatura inválida.");
  }

  const reader = request.body?.getReader();
  if (!reader) throw new ApiError(400, "empty_body", "Evento vazio.");
  let size = 0;
  ...
  while (true) {
    ...
    if (size > 16384) {
      await reader.cancel();
      throw new ApiError(413, "payload_too_large", "Evento muito grande.");
    }
    parts.push(value);
  }
  ...
  try {
    ...
    throw new ApiError(400, "invalid_encoding", "O evento deve usar UTF-8.");
  }

  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
  const sig = Uint8Array.from(signature.match(/../g)!, (byte) => parseInt(byte, 16));
  if (!await crypto.subtle.verify("HMAC", key, sig, new TextEncoder().encode(`${timestamp}.${raw}`))) {
    throw new ApiError(401, "invalid_signature", "Assinatura inválida.");
  }
  ...
  try {
    ...
    throw new ApiError(400, "invalid_json", "Evento inválido.");
  }
  ...
  const db = getDatabase();
  const row = await activationFor(data.organizationId);
  if (!row || row.company_id !== data.companyId || (!row.subscription_id && row.plan_id !== data.planId) || row.request_key !== data.requestKey || (row.subscription_id && row.subscription_id !== data.subscriptionId)) {
    throw new ApiError(409, "activation_mismatch", "Vínculo de ativação incompatível.");
  }

  const connection = await db.prepare("SELECT external_company_id FROM integration_connections WHERE organization_id = ?1 AND provider = 'drap'")
    .bind(data.organizationId).first<{ external_company_id: string }>();
  if (connection && connection.external_company_id !== data.companyId) {
    throw new ApiError(409, "connection_mismatch", "A conexão financeira existente pertence a outra empresa DRAP.");
  }
  ...
  if (previous) {
    if (previous.digest !== digest) throw new ApiError(409, "event_conflict", "ID de evento reutilizado com outro conteúdo.");
    return { received: true, duplicate: true };
  }
  if (data.revision <= row.remote_revision) throw new ApiError(409, "stale_event", "A revisão já foi superada. Consulte o estado atual na DRAP.");

  try {
    await db.batch([
      db.prepare("INSERT INTO drap_activation_events (id, organization_id, digest, received_at) VALUES (?1, ?2, ?3, ?4)")
        .bind(data.eventId, data.organizationId, digest, Date.now()),
