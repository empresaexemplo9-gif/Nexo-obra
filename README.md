...
- Áreas navegáveis de projetos, obras, orçamentos, cronograma, Prancheta, Criador de layout, Comunicação, CRM, financeiro, equipe, tarefas e arquivos.
...
- Criador de layout (24/09/2026): planta com paredes, cômodos com piso, portas, janelas e portão de garagem, e um catálogo de 50 itens na medida de mercado — sofás, camas, mesas, armários, geladeira, fogão, cooktop, máquina de lavar, louças de banheiro, carros (hatch, sedã, SUV, picape), moto, bicicleta, vaga, árvore, piscina e churrasqueira. Prévia 3D ao vivo com paredes levantadas, aberturas, pisos com textura e sombra. Exporta imagem 3D, planta humanizada e prancha de proposta em PDF (A3) direto para a Prancheta, prontas para enviar pela Comunicação. Parte de modelos (casa térrea com garagem, apartamento compacto) ou do zero; salvar exige a versão aberta, então duas pessoas não se sobrescrevem. Consulte [Criador de layout](docs/CRIADOR-DE-LAYOUT.md).
- Editor CAD próprio (fora do menu desde 23/09/2026; a rota `/prancheta/[id]` e a API `/api/studio` continuam): desenho técnico dentro da plataforma, com coordenadas em milímetros e camada por disciplina — a mesma planta serve como layout, elétrico, luminotécnico ou mobiliário sem redesenhar. Paredes, cômodos com área calculada do polígono, portas, janelas, passagens, símbolos elétricos e de iluminação, mobiliário, imagens da biblioteca da empresa, textos, cotas e traço livre; desfazer e refazer, quantitativo derivado do desenho e exportação em SVG e em DXF R12 com medida real. Encaixe em entidade — extremo, interseção, perpendicular, meio e centro, cada um ligável em separado —, trava ortogonal, arrasto de vértice e entrada por teclado no formato de CAD (`3150`, `3150<90`, `@3000,1500`, `3,15m`). Círculo e arco são curva de verdade — guardados por centro, raio e varredura, e exportados como ARC e CIRCLE, não achatados em polilinha. Paralela em um comando para parede, cômodo, traço e arco, com o canto fechado demais chanfrado em vez de virar espeto. Espelhar com giro e sentido corrigidos, matriz retangular e aparar/estender contra qualquer traço do desenho. Biblioteca própria por empresa para mobiliário, texturas, referências e fundo de traçado, com os bytes cifrados no armazenamento. Leitor de DXF: linhas, polilinhas, círculos, arcos, pontos, textos e blocos com inserção aninhada, convertidos para milímetro com inversão do eixo Y, uma camada da prancha por camada do arquivo e palpite de disciplina pelo nome. A unidade vem do cabeçalho do arquivo quando declarada e é confirmada antes de o desenho entrar. Tudo que o leitor não trouxe é dito com nome e contagem. Arquivos DWG são convertidos localmente para DXF pelo motor LibreDWG em WebAssembly, passam pelo mesmo relatório e precisam ser revisados antes de salvar.
